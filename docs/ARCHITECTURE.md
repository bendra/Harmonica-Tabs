...
- Native audio is producer-side rate-limited in `HarmonicaAudioModule` to keep the Expo bridge queue bounded; default `minSendIntervalMs` is `50` and is live-tunable via the temporary Properties "Native send interval ms (debug)" field when using the native fallback. See flow 5C for rationale.
