...
- Single-note detection now uses a narrow first-register anti-aliasing rule: only natural source notes from hole `1` and hole `2` blow may reclaim louder 2x/3x harmonics, and only when the source fundamental plus significant harmonic support is strong enough. Natural notes on holes `9` and `10` also use more forgiving confidence thresholds. The same tuning is mirrored in web and native detectors.
