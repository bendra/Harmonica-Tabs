...
- Current baseline (21 real backing-track clips, `key-samples/results/baseline.json`) now tracks the **live default**, which is **local spectral whitening**: **MIREX 0.486, ~6s window mean 0.460, exact 3/21, playability 0.803.** The harmonic-suppression front-end it replaced scored MIREX 0.400 / window 0.424 / exact 4/21 / playability 0.748 and remains runnable as the `harmonicSuppression` comparison variant in the harness. Whitening improves the headline score and product playability by converting most distant `other` misses into fifth/relative near-misses, but it is not a pure win: exact detections dropped from 4 to 3, so future work should recover tonic precision instead of merely increasing near-miss credit. Confidence still does not reliably predict correctness.
...
- **Spectral whitening shipped as the live chroma front-end (backlog #2).** `chromaWeighting: 'spectralWhitening'` is now `DEFAULT_KEY_DETECTOR_CONFIG`. It divides each FFT bin by the local moving average around that bin (`whitenSpectrum`, radius 8 bins), then normalizes the frame before chroma folding. Measured against the harmonic-suppression baseline on the same 21 clips (`key-samples/results/2026-07-02T17-46-21-469Z.json`): whole-clip MIREX **0.400→0.486 (+0.086)**, ~6s window mean **0.424→0.460 (+0.036)**, playability **0.748→0.803 (+0.054)**, pentatonic containment 0.733→0.857, and `other` misses 6→1. Tradeoff: exact detections fell 4→3 and fifth confusions rose 6→11, meaning whitening is better for playable suggestions but still too often lands on the neighboring functional key rather than the exact tonic.
...
- First front-end experiment (21 local backing-track clips, `key-samples/results/2026-06-29T02-52-52-727Z.json`) tried opt-in `softLogMagnitude` chroma weighting while the live app was still on raw magnitude. It did **not** pass the acceptance bar: raw scored MIREX 0.367 / window mean 0.379 / playability 0.755 / exact 4; soft-log scored MIREX 0.333 / window mean 0.410 / playability 0.755 / exact 3. That rejection remains as a historical result; later harmonic suppression and spectral whitening experiments superseded the raw default.
