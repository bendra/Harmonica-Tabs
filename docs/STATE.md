...
- Current baseline (21 real backing-track clips, `key-samples/results/baseline.json`) now tracks the **live default**, which is **harmonic suppression** (octave-fold HPS): **MIREX 0.400, ~6s window mean 0.424, exact 4/21, playability 0.748.** The raw-magnitude front-end it replaced scored MIREX 0.367 / window 0.379 / playability 0.755 and is still runnable as the `raw` comparison variant in the harness. The detector's earlier **A minor / C major** lean is reduced but not gone, and confidence still does not reliably predict correctness (wrong answers can arrive at conf 0.8+). Remaining chroma-front-end work (stronger whitening/per-bin normalization, bass resolution) is the leading backlog.
- **Harmonic suppression shipped as the live chroma front-end (backlog #1).** `chromaWeighting: 'harmonicSuppression'` is now `DEFAULT_KEY_DETECTOR_CONFIG`. It is an octave-fold harmonic product spectrum (`suppressHarmonics` in `key-detector.ts`, `HPS_HARMONICS = 2`): each FFT bin is replaced by the geometric mean of itself and its octave harmonic, then the per-frame spectrum is renormalized, so true fundamentals reinforce while pure overtones collapse. A harmonic-count sweep on the corpus chose 2: deeper products (3, 4) lifted the ~6s window metric more but *hurt* whole-clip MIREX by manufacturing extra `fifth` confusions. Measured before/after (raw → harmonic suppression): whole-clip MIREX 0.367→**0.400 (+0.033)**, ~6s window mean 0.379→**0.424 (+0.045)**, playability 0.755→0.748 (−0.007, noise-level), confusion `other` 7→6 and `relative` 5→4 at the cost of `fifth` 4→6. This is the first front-end experiment to beat raw on whole-clip MIREX. Playability dipped within noise, so the strict "both metrics up" acceptance bar was a near-miss; shipped on the strength of the clear MIREX gains on both whole-clip and the product-relevant ~6s window.
...
- First front-end experiment (21 local backing-track clips, `key-samples/results/2026-06-29T02-52-52-727Z.json`) tried opt-in `softLogMagnitude` chroma weighting while keeping the live app on raw magnitude. It did **not** pass the acceptance bar: raw scored MIREX 0.367 / window mean 0.379 / playability 0.755 / exact 4; soft-log scored MIREX 0.333 / window mean 0.410 / playability 0.755 / exact 3. Keep raw as the live default and prefer harmonic/overtone suppression or stronger per-bin whitening as the next measured experiment.
