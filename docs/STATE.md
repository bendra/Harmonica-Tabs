...
- Key-detection accuracy is measured before it is tuned. `scripts/key-detect-offline.ts` is an evaluation harness (run `npm run key-detect-offline -- ./key-samples`) that scores the detector against a labelled corpus and reports a **MIREX-style weighted score** (exact 1.0 / fifth 0.5 / relative 0.3 / parallel 0.2), a **confusion breakdown** (exact/fifth/relative/parallel/other — which localizes the fault: fifth/relative confusions point at the K–S profiles/weighting, "other" at a broken chroma front-end), per-quality/per-tonic accuracy, **confidence calibration** buckets (validating `KEY_CONFIDENCE_MIN`), a per-clip normalized chroma sparkline, and both whole-clip and ~6s sliding-window scores (the app only ever hears ~6s). It writes machine-readable aggregates to `key-samples/results/<timestamp>.json` (commit one as `baseline.json` for regression diffs). The corpus lives in `key-samples/` with labels in `key-samples/labels.json` (or encoded in filenames as `song__D_major.wav`); the audio is gitignored (copyright/size) while labels and results are tracked. The decision to build measurement first (and **not** change the algorithm in the same pass) is deliberate: the current chroma front-end is naive (linear FFT bins, round-to-nearest-MIDI folding, no harmonic/whitening/tuning handling), and K–S detectors fail in characteristic ways, so the confusion profile must guide which fix to try. See `docs/TODO.md` for the prioritized improvement backlog.
- First baseline (14 real backing-track clips, `key-samples/results/baseline.json`): **MIREX 0.31, exact 2/14, minor keys 0/7.** The detector systematically collapses onto **A minor / C major** regardless of the true key (the chroma sparklines peak on A/C almost everywhere), and confidence does not predict correctness (wrong answers routinely arrive at conf 0.8+). This "A-minor attractor" points at the **chroma front-end** (overtone bleed + no spectral normalization), not the K–S profiles — so harmonic suppression and spectral whitening lead the backlog.
- The harness also reports a **playability lens** beside MIREX (`scripts/key-detect-offline.ts`, `playabilityFor`). Rationale: MIREX scores key *identification* closeness, but a harp player cares about *note compatibility*, and the two disagree — a `relative` miss (C major→A minor) shares all 7 notes (fully playable) yet scores only 0.3 in MIREX. Playability = the fraction of notes a player would actually play over the **detected** key (the app's recommended scale via `recommendedForQuality` — mixolydian/dorian; plus a secondary minor-pentatonic "blues" indicator) that fall inside the **true** key's diatonic scale. Baseline playability ≈ 0.76 (pentatonic ≈ 0.80), which quantifies the real observation that the "wrong" keys usually still *sound* OK to play over. **Caveat (kept honest):** this lowers the measured cost of note-sharing misses but does **not** excuse the detector — `parallel` misses clash on the third, the minor→major confusions genuinely sound wrong, and the A-minor attractor lands somewhere playable only by luck. (Note: even exact *major* matches score ~0.86 not 1.0, because the recommended cross-harp mixolydian's bluesy ♭7 sits just outside the strict major scale — an expected artifact, read playability comparatively across categories rather than as an absolute.) MIREX stays the literature-comparable number; playability is the product-truth number; both are shown.
