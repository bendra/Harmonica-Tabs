...
- Key detection (`Find song key` on `Scales`) follow-ups:
  - **(Done) Labelled corpus + baseline established.** 21 clips in `key-samples/`, baseline in `key-samples/results/baseline.json` (now tracks the live default — local spectral whitening + bin interpolation — at MIREX 0.590, window mean 0.545, exact 7/21, playability 0.844; prior plain spectral whitening scored 0.486 / 0.460 / 3 exact / 0.803). Worth growing toward ~20–40 clips (more tonics, more minor keys) for a firmer signal. No algorithm change ships without a measured before/after on this corpus.
  - **Report the playability lens beside MIREX on every before/after.** The harness now also prints a playability score (notes the player would play over the *detected* key vs. the *true* key's scale; see `playabilityFor` in `harmonica-tabs/scripts/key-detect-offline.ts`) — baseline ≈ 0.76. A front-end fix should move *both* MIREX and playability up; watch playability so a change that raises MIREX doesn't quietly make the consonant near-misses worse.
  - **(Rejected experiment) Soft log-magnitude weighting did not pass the acceptance bar.** `harmonica-tabs/scripts/key-detect-offline.ts` still compares the raw-magnitude chroma against the opt-in `softLogMagnitude` experiment in one report. On the 21 local clips (`key-samples/results/2026-06-29T02-52-52-727Z.json`), raw scored MIREX 0.367 / window mean 0.379 / playability 0.755 / exact 4; soft-log scored MIREX 0.333 / window mean 0.410 / playability 0.755 / exact 3. Since whole-clip MIREX fell and playability did not improve, soft log remains rejected.
  ...
  - **Improvement backlog — order confirmed by the baseline's confusion profile** (the A-minor attractor + ubiquitous A/C chroma peaks point squarely at the chroma front-end, so #1–#2 lead):
    1. **(Done)** Harmonic/overtone suppression in the chroma. `chromaWeighting: 'harmonicSuppression'` (octave-fold HPS, `HPS_HARMONICS = 2`) measured raw → HPS gains of whole-clip MIREX +0.033 / ~6s window +0.045 (playability −0.007, noise-level). It cut `other`/`relative` misses but added some `fifth` confusions. See `docs/STATE.md`.
    2. **(Done)** Spectral whitening / per-bin normalization beyond simple soft log-magnitude. `chromaWeighting: 'spectralWhitening'` measured harmonic suppression → whitening gains of whole-clip MIREX +0.086, ~6s window +0.036, playability +0.054, and `other` misses 6→1. Tradeoff: exact detections fell 4→3 and fifth confusions rose 6→11.
    3. **(Done — shipped as live default)** Bass-resolution bin interpolation. `chromaWeighting: 'spectralWhiteningInterpolated'` measured plain whitening → interpolated gains of whole-clip MIREX +0.105, ~6s window +0.086, playability +0.041, exact detections 3→7, and fifth confusions 11→9. It keeps the frame size and adds only clamped parabolic FFT-bin interpolation before pitch-class rounding.
