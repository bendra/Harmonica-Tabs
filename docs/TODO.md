...
- Key detection (`Find song key` on `Scales`) follow-ups:
  - **(Done) Labelled corpus + baseline established.** 21 clips in `key-samples/`, baseline in `key-samples/results/baseline.json` (now tracks the live default — harmonic suppression — at MIREX 0.400, window mean 0.424, exact 4/21, playability 0.748; the prior raw front-end scored 0.367 / 0.379 / 0.755). Worth growing toward ~20–40 clips (more tonics, more minor keys) for a firmer signal. No algorithm change ships without a measured before/after on this corpus.
  ...
  - **(Rejected experiment) Soft log-magnitude weighting did not pass the acceptance bar.** `harmonica-tabs/scripts/key-detect-offline.ts` now compares the live raw-magnitude chroma against an opt-in `softLogMagnitude` experiment in one report. On the current 21 local clips (`key-samples/results/2026-06-29T02-52-52-727Z.json`), raw scored MIREX 0.367 / window mean 0.379 / playability 0.755 / exact 4; soft-log scored MIREX 0.333 / window mean 0.410 / playability 0.755 / exact 3. Since whole-clip MIREX fell and playability did not improve, keep the live detector on raw magnitude.
  ...
  - **Improvement backlog — order confirmed by the baseline's confusion profile** (the A-minor attractor + ubiquitous A/C chroma peaks point squarely at the chroma front-end, so #1–#2 lead):
    1. **(Done — shipped as live default)** Harmonic/overtone suppression in the chroma. `chromaWeighting: 'harmonicSuppression'` (octave-fold HPS, `HPS_HARMONICS = 2`) is now the default; measured raw → HPS gains of whole-clip MIREX +0.033 / ~6s window +0.045 (playability −0.007, noise-level). It cut `other`/`relative` misses but added some `fifth` confusions, so the next front-end pass (#2) should watch that the fifth axis doesn't worsen further. See `docs/STATE.md`.
    2. Spectral whitening / per-bin normalization beyond simple soft log-magnitude — stops drums & distortion from dominating the chroma.
