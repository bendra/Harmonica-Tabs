...
- Re-test detector tuning on physical web + native devices for C, E, and G harps after the latest first-register anti-aliasing pass and hole `9`/`10` confidence-threshold adjustments, and compare any remaining native-only issues with the new one-time capture diagnostics.
