...
- Spike a WebView-based audio detector on iOS before more blind native capture tuning; see `docs/WEBVIEW_AUDIO_SPIKE.md`. iPad Safari over HTTPS detects the known high-note cases correctly, but the native AVAudioEngine path still shows octave-low frames.
...
- Decide the fate of the temporary native-audio "Send interval ms (debug)" Properties control: either remove it now that the floor (50ms) is chosen and baked in as the default, or formally keep it as a hidden tuning knob (currently wired through 8 files; see `docs/ARCHITECTURE.md` flow 5C).
