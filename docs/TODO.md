...
- Key detection (`Find song key` on `Scales`) follow-ups:
  - **(Done) Labelled corpus + baseline established.** 21 clips in `key-samples/`, baseline in `key-samples/results/baseline.json` (now tracks the live default — local spectral whitening — at MIREX 0.486, window mean 0.460, exact 3/21, playability 0.803; prior harmonic suppression scored 0.400 / 0.424 / 4 exact / 0.748). Worth growing toward ~20–40 clips (more tonics, more minor keys) for a firmer signal. No algorithm change ships without a measured before/after on this corpus.
  ...
  - **Improvement backlog — order confirmed by the baseline's confusion profile** (the A-minor attractor + ubiquitous A/C chroma peaks point squarely at the chroma front-end, so #1–#2 lead):
    ...
    2. **(Done — shipped as live default)** Spectral whitening / per-bin normalization beyond simple soft log-magnitude. `chromaWeighting: 'spectralWhitening'` measured harmonic suppression → whitening gains of whole-clip MIREX +0.086, ~6s window +0.036, playability +0.054, and `other` misses 6→1. Tradeoff: exact detections fell 4→3 and fifth confusions rose 6→11, so the next improvement should recover tonic precision rather than only making misses more playable.
